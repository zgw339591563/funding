...
<%-- 						        <c:if test="${userPage.pageno == 1}">
						        ...
						        <c:if test="${userPage.pageno != 1}">
						        <li><a href="#" onclick="changePageno(${userPage.pageno-1})">上一页</a></li>
						        </c:if>								
								<c:forEach begin="1" end="${userPage.totalno}" var="num">
								    <li
								        <c:if test="${userPage.pageno eq num}">
								        class="active"
								        </c:if>
								    ><a href="#" onclick="changePageno(${num})">${num}</a></li>
								</c:forEach>
<!-- 								<li class="active"><a href="#">1 <span class="sr-only">(current)</span></a></li>
								...
								<c:if test="${userPage.pageno != userPage.totalno}">
								    <li><a href="#" onclick="changePageno(${userPage.pageno+1})">下一页</a></li>
								</c:if>
								<c:if test="${userPage.pageno == userPage.totalno}">
	...
	<script src="${APP_PATH}/script/render.js"></script>
        <script type="text/javascript">
            ...
            function deleteUser(id, loginacct) {
    			layer.confirm("删除账号为"+loginacct+"用户, 是否继续？",  {icon: 3, title:'提示'}, function(cindex){
    				// 删除数据
    				$.ajax({
